...
namespace UnityStandardAssets.ImageEffects
{
	...
	[AddComponentMenu("Image Effects/Rendering/Screen Space Ambient Occlusion Optimized(Slightly)")]
	public class ScreenSpaceAmbientOcclusionOptimized : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			CreateMaterials();
			if (!m_SSAOMaterial || m_SSAOMaterial.passCount != 8)
			{
				m_Supported = false;
				base.enabled = false;
			}
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			RenderTexture temporary = RenderTexture.GetTemporary(source.width / m_Downsampling, source.height / m_Downsampling, 0, RenderTextureFormat.RFloat, RenderTextureReadWrite.Linear);
			Graphics.Blit(null, temporary, m_SSAOMaterial, 6);
			m_SSAOMaterial.SetTexture("_ScaledCameraDepth", temporary);
			...
			if (flag)
			{
				RenderTexture temporary2 = RenderTexture.GetTemporary(source.width, source.height / m_Downsampling, 0);
				...
				Graphics.Blit(null, temporary2, m_SSAOMaterial, 4);
				...
				m_SSAOMaterial.SetTexture("_SSAO", temporary2);
				Graphics.Blit(source, destination, m_SSAOMaterial, 7);
